...
- release-pinned README:
  `https://github.com/FoxsterDev/xuunity-mcp/blob/v0.3.76/README.md`
...
- If a host helper directory already exists under `~/.codex-tools`,
  ...
  have been checked against v0.3.76 and stale files have been refreshed from the
...
```text
Set up XUUnity Light Unity MCP release v0.3.76 from
...
unity_status_summary. Require mcp_server_info.version=0.3.76 in that live result.
...
2. compare the requested v0.3.76 release with the manifest package pin, installed
   helper version/source, and current client launcher before executing an
   existing helper
3. use `setup-plan` from the approved v0.3.76 source before any mutation
...
the requested release. For an install or upgrade to v0.3.76, inspect before use
and refresh from the approved v0.3.76 source after approval.
...
or perform a v0.3.76 upgrade: an old refresh launcher can remain pinned to its
old source checkout. Run `setup-plan` from the approved v0.3.76 source checkout,
...
For package-level verification after upgrading to `v0.3.76`, run:
