...
| --- | --- | --- | --- | --- |
| 2026-07-06 | `2026-07-06_first_open_6000_upgrade_apiupdate_modal_and_console_source_retro.md` | First-open 6000 upgrade: API-Updater modal deadlock when GUI opened without `-accept-apiupdate`, plus `console.grep source=console` false-empty for compile errors after a domain reload | durable fix set implemented in v0.3.40; compact envelope backlog remains | Implemented modal-block health hypothesis + `relaunch_noninteractive_accept_apiupdate` recovery, path-backed `editor_log` default for grep, explicit Console false-empty warnings, and first-open `-accept-apiupdate` command construction. Deferred: broader compact idle-timeout/transport envelopes. |
| 2026-07-06 | `2026-07-06_bridge_declared_not_enabled_first_open_install_retro.md` | Install: package declared but bridge `bridge_disabled` on first open; project-only enable needs separate `init --enable-project` + editor reopen; `setup-plan` bundles user-scope client-config mutations | durable fix set implemented in v0.3.40 | Implemented project-scoped bridge config auto-enable during `ensure-ready --open-editor`, kept setup-plan project setup mutations free of user-level client config writes, and clarified optional client wiring review targets. |
| 2026-07-06 | `2026-07-06_batchmode_blind_to_editor_startup_reconcilers_retro.md` | Batchmode/player compile does not execute `[InitializeOnLoad]`/editor-startup reconcilers and excludes editor+test assemblies (green compile != editor clean); offline `health_probe` `editor_log_diagnosis` surfaced a prior-session compile block without a freshness qualifier | active candidate; `SMOKE_TESTS` compile-gate scope-limit + editor-startup-clean gate added in this change | Companion to the same-day `first_open_6000_upgrade_apiupdate_modal_and_console_source` retro. Remaining candidates: offline-diagnosis freshness qualifier in health output, and an `AGENT_WORKFLOWS` onboarding note. |
