...
| 2026-06-02 | `2026-06-02_token_efficiency_response_envelope_retro.md` | token efficiency response envelope retro | partially implemented; follow-up remains | Scenario verdict compacting shipped in `v0.3.32`; this run added compact-by-default MCP tool summaries for refresh, compile, build-config compile, and direct test responses with `includeFullPayload=true` full-payload opt-in. Remaining response-envelope work: status-summary slimming, batch/multi-project compact ceilings, token ledger, and fast-path profiles. |
...
> `includeFullPayload=true`. Status-summary slimming, batch/multi-project
