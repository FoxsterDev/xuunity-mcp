...
| --- | --- | --- | --- |
| 2026-05-23 | `XUUNITY_MCP_OPTIONAL_CAPABILITY_SETUP_WIZARD_DESIGN_2026-05-23.md` | `implemented; host-validated` | Added optional Test Framework assembly, package metadata without hard Test Framework dependency, capability statuses, setup plan/apply/validate/install CLI, setup MCP tools, approved Unity-side Test Framework install operation, batch capability preflight, and explicit handling for already-installed suitable/old/upgrade-recommended Test Framework versions. Validation: `scripts/testing/run_host_python_tests.sh` passed 115 tests before the follow-up; targeted setup-wizard tests now cover existing dependency upgrade behavior. Live Unity version matrix remains follow-up before release tag. |
