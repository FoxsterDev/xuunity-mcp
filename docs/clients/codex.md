...
This guide is pinned to XUUnity release `v0.3.80` from the canonical repository
...
v0.3.80 source through the native wrapper instead of executing an old helper or
requiring Git Bash:
...
`.source_root` with v0.3.80. Do not execute stale helper files. Refresh them
from the approved v0.3.80 source after the preflight review is approved.
...
the installed version/source against v0.3.80 before executing it for this setup.
...
1. `unity_status_summary` and require `mcp_server_info.version=0.3.80`
