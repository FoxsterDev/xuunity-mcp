...
| Area | Status | Caveat |
| --- | --- | --- |
| Current package source | `Git UPM available` | Source package is `v0.3.38` at `packages/com.xuunity.light-mcp`; `v0.3.38` is the current Git UPM release target. OpenUPM publication is still pending. |
| macOS host tools | `Repo-verified` | Host Python unittest suite passed for `v0.3.36`: `279` tests with one expected skip. |
