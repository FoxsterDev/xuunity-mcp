...
| Area | Status | Caveat |
| --- | --- | --- |
| Current package source | `Release tag prepared` | Source package is `v0.3.20` at `packages/com.xuunity.light-mcp`; push the local release tag to `origin` before using it from Git UPM consumers. OpenUPM publication is still pending. |
