...
for MCP-capable AI agents. The current source line is `v0.3.34`.
...
Latest source validation for `v0.3.34`:

| Area | Evidence | Result |
| --- | --- | --- |
| Package metadata | `packages/com.xuunity.light-mcp/package.json` | `name=com.xuunity.light-mcp`, `version=0.3.34`, `unity=2021.3`, no hard Test Framework dependency |
| Host Python tests | `python3 -m unittest discover -s tests` | `267` tests passed for `v0.3.34`, with one expected skip |
...
| Release prodmode check | Temporary Unity project pinned to `#v0.3.34` | Run after pushing the release tag so `prodmode` can verify the tag on `origin` and write the matching Git UPM dependency before consumer closeout. |
| Multi-project compile matrix | Private multi-project consumer validation | `9/9` projects, `38/38` lanes, `0` failures |
| Git tag visibility | Git refs | Release tag `v0.3.34` is prepared locally and must be pushed to `origin` for Git UPM consumers. |
...
- `prodmode` pins the Unity project to the published release tag that matches
  the package version, for example `#v0.3.34`.
