...
for MCP-capable AI agents. The current released source line is `v0.3.77`.
...
Latest release and current-source validation for `v0.3.77`:

| Area | Evidence | Result |
| --- | --- | --- |
| Package metadata | `packages/com.xuunity.light-mcp/package.json` | `name=com.xuunity.light-mcp`, `version=0.3.77`, `unity=2021.3`, no hard Test Framework dependency |
| Host Python tests | `scripts/testing/run_host_python_tests.sh` (release checks plus full discovery) | Full discovery passed `1076` tests with `14` expected platform skips, including live TCP loopback transport coverage. |
...
| Public site checks | `scripts/testing/run_site_ui_checks.sh` | Public site Playwright checks passed for `v0.3.77`: `42/42`. |
...
| Git tag visibility | Remote Git refs | Release tag `v0.3.77` is the current Git UPM release target; remote publication requires an authenticated push. |
...
- `prodmode` pins the Unity project to the published release tag that matches
  the package version, for example `#v0.3.77`.
