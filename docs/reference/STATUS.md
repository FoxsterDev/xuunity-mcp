...
for MCP-capable AI agents. The current released source line is `v0.3.41`.
...
Latest source validation for `v0.3.41`:

| Area | Evidence | Result |
| --- | --- | --- |
| Package metadata | `packages/com.xuunity.light-mcp/package.json` | `name=com.xuunity.light-mcp`, `version=0.3.41`, `unity=2021.3`, no hard Test Framework dependency |
| Host Python tests | `python3 -m unittest discover -s tests` | `309` tests passed for `v0.3.41`, with one expected skip |
| Compact MCP envelopes | Changelog and regression coverage for `0.3.32`-`0.3.41` | Scenario decision verdicts, compact operation/readiness/status summaries, authoritative post-settle compile/test/refresh fields, editor-log identity, scenario step-payload opt-ins, PlayMode already-playing stale-risk summaries, deterministic scene-open setup, opt-in compact batch helper output, safer `Editor.log` console grep/tail defaults, and compact transport/idle timeout errors are documented with full-payload recovery. |
...
| Public site checks | `scripts/testing/run_site_ui_checks.sh` | Public site Playwright checks passed for `v0.3.41`: `39/39`. |
...
| Git tag visibility | Remote Git refs | Release tag `v0.3.41` is the current Git UPM release target; remote publication requires an authenticated push. |
...
- `prodmode` pins the Unity project to the published release tag that matches
  the package version, for example `#v0.3.41`.
