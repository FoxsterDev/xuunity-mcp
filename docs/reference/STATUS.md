...
Latest source validation for `v0.3.43`:

| Area | Evidence | Result |
| --- | --- | --- |
| Package metadata | `packages/com.xuunity.light-mcp/package.json` | `name=com.xuunity.light-mcp`, `version=0.3.43`, `unity=2021.3`, no hard Test Framework dependency |
...
| Git tag visibility | Remote Git refs | Release tag `v0.3.43` is the current Git UPM release target; remote publication requires an authenticated push. |
...
{
  "dependencies": {
    "com.xuunity.light-mcp": "https://github.com/FoxsterDev/xuunity-mcp.git?path=/packages/com.xuunity.light-mcp#v0.3.43"
  }
}
```
...
- `prodmode` pins the Unity project to the published release tag that matches
  the package version, for example `#v0.3.43`.
