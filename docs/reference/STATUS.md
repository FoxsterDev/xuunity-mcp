...
for MCP-capable AI agents. The current released source line is `v0.3.74`.
...
Latest release and current-source validation for `v0.3.74`:
...
| Public site checks | `scripts/testing/run_site_ui_checks.sh` | Public site Playwright checks passed for `v0.3.74`: `42/42`. |
...
| Git tag visibility | Remote Git refs | Release tag `v0.3.74` is the current Git UPM release target; remote publication requires an authenticated push. |
...
- `prodmode` pins the Unity project to the published release tag that matches
  the package version, for example `#v0.3.74`.
