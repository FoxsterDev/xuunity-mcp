...
```text
Set up XUUnity Light Unity MCP release v0.3.79 from the canonical repository
https://github.com/FoxsterDev/xuunity-mcp for /path/to/UnityProject, follow
https://github.com/FoxsterDev/xuunity-mcp/blob/v0.3.79/README.md. Before executing
an existing helper, compare its version and .source_root with v0.3.79 and refresh
...
and run unity_status_summary. Require mcp_server_info.version=0.3.79 in that live
...
1. Read the release-pinned
   `https://github.com/FoxsterDev/xuunity-mcp/blob/v0.3.79/README.md`, its
...
9. After any helper or client-config change, restart or refresh the client,
   ...
   `mcp_server_info.version=0.3.79`. Helper-only validation does
...
- Requested package release: <v0.3.79>
...
- Required live proof after restart: <server listed, tools listed, unity_status_summary healthy with mcp_server_info.version=0.3.79>
...
- Required release: v0.3.79
- Release README: https://github.com/FoxsterDev/xuunity-mcp/blob/v0.3.79/README.md
...
- Reuse an existing helper install directory only after checking its installed
  version, `.source_root`, and refresh launcher against v0.3.79. Do not execute
  stale helper files; refresh them from the approved v0.3.79 source first.
...
2. Use the canonical source repository
   https://github.com/FoxsterDev/xuunity-mcp. If its v0.3.79 source is missing
...
4. Show a short preflight review with:
   ...
   - requested v0.3.79 package release and current manifest pin
...
6. After approval, refresh the host helper whenever its version/source does not
   match v0.3.79. Reuse the install directory, not stale files:
   - POSIX: `bash init_xuunity_light_unity_mcp.sh`
   - native Windows Codex: set
     `XUUNITY_LIGHT_UNITY_MCP_INSTALL_TARGET=codex`, then run
     `.\xuunity_light_unity_mcp.cmd server-help` from the v0.3.79 source
...
{
  "dependencies": {
    "com.xuunity.light-mcp": "https://github.com/FoxsterDev/xuunity-mcp.git?path=/packages/com.xuunity.light-mcp#v0.3.79"
  }
}
```
...
itself current. Before executing an existing helper for a v0.3.79 setup, compare
its installed version and `.source_root` with the approved v0.3.79 source and
