...
resolve_python_bin() {
  if [[ -n "${PYTHON:-}" ]]; then
    local python_value="$PYTHON"
    if [[ "$python_value" == *\\* ]]; then
      python_value="${python_value//\\//}"
    fi
    if [[ "$python_value" == "py -3" ]] && command -v py >/dev/null 2>&1; then
      command -v py
      return 0
    fi
    if [[ "$python_value" != */* && "$python_value" != *" "* ]] && command -v "$python_value" >/dev/null 2>&1; then
      command -v "$python_value"
      return 0
    fi
    printf '%s\n' "$python_value"
    return 0
